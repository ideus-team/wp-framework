...
if ( ! class_exists( 'Breadcrumbs' ) ) {
	...
	class Breadcrumbs {
		 ...
		 */
		public function get_crumbs( $sep, $l10n, $args ) {
			...
			if ( 1 ) {
				...
				} elseif ( 'rdf.data-vocabulary.org' === $mark ) {
					// rdf.
					$mark = array(
			...
			if ( is_front_page() ) {
				...
				$out = $this->add_title( $this->page_crumbs( $post ), $post );
			} else {
				...
				if ( is_singular() ) {
					...
					if ( $taxonomies ) {
						...
						if ( ! empty( $arg->priority_tax ) ) {
							usort(
								$taxonomies,
								function ( $a, $b ) use( $arg ) {
									$a_index = array_search( $a, $arg->priority_tax, true );
									if ( false === $a_index ) {
										$a_index = 9999999;
									}

									$b_index = array_search( $b, $arg->priority_tax, true );
									if ( false === $b_index ) {
										$b_index = 9999999;
									}
					...
					if ( isset( $save_post ) ) {
						$post = $save_post; // повернемо назад (для вкладень).
					}
				}
				...
				if ( $term && isset( $term->term_id ) ) {
					...
					if ( is_attachment() ) {
						...
						} elseif ( ! $out = apply_filters( 'attachment_tax_crumbs', '', $term, $this ) ) {
								$_crumbs    = $this->tax_crumbs( $term, 'self' );
								$parent_tit = sprintf( $linkpatt, get_permalink( $post->post_parent ), get_the_title( $post->post_parent ) );
								$_out       = implode( $sep, array( $_crumbs, $parent_tit ) );
								$out        = $this->add_title( $_out, $post );
						}
					} elseif ( is_single() ) {
						// single.
						if ( ! $out = apply_filters( 'post_tax_crumbs', '', $term, $this ) ) {
							$_crumbs = $this->tax_crumbs( $term, 'self' );
							$out     = $this->add_title( $_crumbs, $post );
						}
					} elseif ( ! is_taxonomy_hierarchical( $term->taxonomy ) ) {
						// не деревоподібна таксономія (теги).
						if ( is_tag() ) {
							// тег.
							$out = $this->add_title( '', $term, sprintf( $loc->tag, esc_html( $term->name ) ) );
						} elseif ( is_tax() ) {
							// таксономія.
							$post_label = $ptype->labels->name;
							$tax_label  = $GLOBALS['wp_taxonomies'][ $term->taxonomy ]->labels->name;
							$out        = $this->add_title( '', $term, sprintf( $loc->tax_tag, $post_label, $tax_label, esc_html( $term->name ) ) );
						}
					} elseif ( ! $out = apply_filters( 'term_tax_crumbs', '', $term, $this ) ) {
						// деревоподібна таксономія (рубрики).
						$_crumbs = $this->tax_crumbs( $term, 'parent' );
						$out     = $this->add_title( $_crumbs, $term, esc_html( $term->name ) );
					}
					...
					if ( is_post_type_hierarchical( $parent->post_type ) ) {
						...
						$parent_crumbs = $this->page_crumbs( $parent );
			...
			if ( '' === $home_after ) {
				...
				if ( $ptype && $ptype->has_archive && ! in_array( $ptype->name, array( 'post', 'page', 'attachment' ), true ) && ( is_post_type_archive() || is_singular() || ( is_tax() && in_array( $term->taxonomy, $ptype->taxonomies, true ) ) ) ) {
		...
		/**
		 * [page_crumbs description]
		 *
		 * @param  [type] $post [description]
		 * @return [type]       [description]
		 */
		private function page_crumbs( $post ) {
		...
		/**
		 * [tax_crumbs description]
		 *
		 * @param  [type] $term       [description]
		 * @param  string $start_from [description]
		 * @return [type]             [description]
		 */
		private function tax_crumbs( $term, $start_from = 'self' ) {
			$termlinks = array();
			$term_id   = ( 'parent' === $start_from ) ? $term->parent : $term->term_id;
		 ...
		 */
		private function add_title( $add_to, $obj, $term_title = '' ) {
